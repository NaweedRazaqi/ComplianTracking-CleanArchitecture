...
namespace Clean.Application.ProcessTrackings.Commands
{
    ...
    public class SaveProcessTracksCommandHandler : IRequestHandler<SaveProcessTracksCommand, List<SearchedProcessTracks>>
    {
        ...
        public SaveProcessTracksCommandHandler(BaseContext context, IMediator mediator,ICurrentUser  current,IEnumerable<IProcessChangeListener> listeners, AppDbContext _context)
        {
            Context = context;
            Mediator = mediator;
            CurrentUser = current;
        ...
        public async Task<List<SearchedProcessTracks>> Handle(SaveProcessTracksCommand request, CancellationToken cancellationToken)
        {
            ...
            var complaintype = _context.CaseRegister.Where(e => e.Id == request.RecordId).Select(e => e.ComplainTypeId).SingleOrDefault();
            var lastprocess = _context.Process.Where(p => p.ComplainTypeId == complaintype).OrderByDescending(p => p.Id).FirstOrDefault();
          //  var currentprocess = Context.ProcessTracking.Where(pt => pt.RecordId == request.RecordId && pt.ProcessId == request.ProcessId).Select(pt => pt.ProcessId).FirstOrDefault();
           if(lastprocess.Id == request.ReferedProcessId)
            {
                 App.Domain.Entity.Complains.Registeration casereg = _context.CaseRegister.Where(e => e.Id == request.RecordId).SingleOrDefault();
                casereg.IsLastStep = true;
